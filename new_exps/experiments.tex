...
\label{subsec:exp6_multi_shot}

\subsubsection{Objective}

Compare three approaches that use multiple timesteps of measurement data:
\begin{enumerate}
    \item \textbf{Single-snapshot IAUKF}: 200 sequential steps, 1 measurement at a time (baseline)
    \item \textbf{Multi-snapshot IAUKF}: 40 steps with 5 snapshots simultaneously (augmented state, Section IV.C)
    \item \textbf{Graph-Mamba}: Single forward pass, 50 timesteps at once (deep learning)
\end{enumerate}

This provides a fair comparison of multi-shot estimation methods.

\subsubsection{Methodology}

All three methods process approximately 50 timesteps of data:
\begin{itemize}
    \item Single IAUKF: 200 steps $\times$ 1 measurement = 200 measurements
    \item Multi IAUKF: 40 steps $\times$ 5 snapshots = 200 measurements  
    \item Graph-Mamba: 1 pass $\times$ 50 timesteps = 50 measurements
\end{itemize}

The multi-snapshot IAUKF uses an augmented state vector (Eq 32-38 in paper):
\[
\mathbf{X}_k = [\mathbf{x}_1, \mathbf{x}_2, \dots, \mathbf{x}_t, \mathbf{p}_k]^T
\]
where $t=5$ snapshots are processed simultaneously, increasing state dimension by 5$\times$.

\subsubsection{Results}

\begin{table}[h]
\centering
\caption{Experiment 6: Multi-Shot Estimation Comparison}
\label{tab:exp6_multishot}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{R Error (\%)} & \textbf{X Error (\%)} & \textbf{Time (ms)} \\
\midrule
Single IAUKF (200$\times$1) & 1.63 $\pm$ 1.35 & 7.17 $\pm$ 4.96 & 2,562 \\
Multi IAUKF (40$\times$5) & 3.59 $\pm$ 2.04 & 8.29 $\pm$ 0.37 & 24,088 \\
\textbf{Graph-Mamba (1$\times$50)} & \textbf{1.69 $\pm$ 0.01} & \textbf{4.92 $\pm$ 0.04} & \textbf{108} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{Multi-Snapshot IAUKF Limitation}: Despite using fewer iterations (40 vs 200), the augmented state space (5$\times$ larger) makes each iteration significantly more expensive. The multi-snapshot approach is \textbf{9.4$\times$ slower} than single-snapshot (24s vs 2.5s).
    
    \item \textbf{Graph-Mamba Advantage}: Achieves comparable accuracy to single-snapshot IAUKF (1.69\% vs 1.63\%) but with \textbf{23.8$\times$ speedup} (108 ms vs 2,562 ms).
    
    \item \textbf{Data Efficiency}: Graph-Mamba uses only 50 timesteps vs 200 for IAUKF variants, yet achieves similar or better accuracy, demonstrating efficient temporal feature extraction through the SSM.
\end{enumerate}

\textbf{Implications:} The multi-snapshot approach (Section IV.C) is theoretically appealing but computationally expensive due to the enlarged state space. Graph-Mamba's neural architecture achieves better time-data trade-off through learned representations.
...
\toprule
\textbf{Exp} & \textbf{Name} & \textbf{Key Result} & \textbf{IAUKF} & \textbf{Graph-Mamba} & \textbf{Winner} \\
\midrule
1 & Basic Performance & Accuracy (main) & 3.90\% & 1.63\% & GM 58\% better \\
2 & Dynamic Tracking & Step response & Good & Good & Similar \\
3 & Low Observability & Convergence (1 PMU) & 0\% & 100\% & GM reliable \\
4 & Speed & Time (118-bus) & 720 ms & 2.33 ms & GM 308$\times$ faster \\
5 & Robustness & Cauchy noise & DIVERGED & 1.73\% & GM robust \\
6 & Multi-Shot & Time (50 timesteps) & 2,562 ms & 108 ms & GM 24$\times$ faster \\
7 & Generalization & 118-bus transfer & 228M\% & 75\% & GM stable \\
...
\begin{enumerate}
    ...
    \item \textbf{Multi-Shot Estimation (Exp 6)}: When comparing multi-timestep methods, Graph-Mamba (108 ms) is 24$\times$ faster than multi-snapshot IAUKF (2,562 ms) with comparable accuracy
    
    \item \textbf{Generalization (Exp 7)}: Graph-Mamba provides stable 75\% error on untrained 118-bus system where IAUKF diverges (228M\% error)
